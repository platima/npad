...
Window *ui_platform_create_main_window(void) {
    ...
    window->hwnd = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_ACCEPTFILES | WS_EX_WINDOWEDGE,
...
#define ID_HELP_ABOUT 2301
    ...
    Window *ui_platform_create_main_window(void) {
        ...
        // Create status bar
        window->status_hwnd =
            CreateWindowEx(0, STATUSCLASSNAME, NULL, WS_CHILD | WS_VISIBLE | SBARS_SIZEGRIP, 0, 0,
                           0, 0, window->hwnd, (HMENU) ID_STATUS_BAR, g_hinstance, NULL);
        ...
        if (!window->haccel) {
            NPAD_ERROR_WARNING(
                NPAD_ERROR_SYSTEM, GetLastError(), "Accelerator table creation",
                "Failed to create accelerator table - keyboard shortcuts will not work");
            // Continue without accelerators rather than failing completely
        }
