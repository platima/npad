...
bool editor_save_file(void)
{
    ...
    if (!g_editor.main_window) return false;
    ...
    if (success) {
        ...
        char error_msg[512];
        snprintf(error_msg, sizeof(error_msg), 
                "Could not save file: %s", g_editor.current_file);
        ui_show_message_box(g_editor.main_window, "Error", error_msg, false);
    }
...
{
    if (!filename || !g_editor.main_window) return false;
...
{
    if (!g_editor.is_modified) return true;
    
    const char* filename = g_editor.current_file ? 
        g_editor.current_file : "Untitled";
    
    char message[512];
    snprintf(message, sizeof(message), 
            "Do you want to save changes to %s?", filename);
...
{
    if (!g_editor.main_window) return;
    ...
    snprintf(title, sizeof(title), "%s%s - npad", 
             g_editor.is_modified ? "*" : "", display_name);
