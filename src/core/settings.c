...
{
    return settings_set_int("window_x", x) &&
           settings_set_int("window_y", y) &&
           settings_set_int("window_width", width) &&
           settings_set_int("window_height", height) &&
...
{
    if (!x || !y || !width || !height || !maximized) return false;
